...
public:
    AnimatedTexture() noexcept : 
        mPaused(false), mFrame(0), mFrameCount(0), mTextureWidth(0), mTextureHeight(0),
        mTimePerFrame(0.0f), mTotalElapsed(0.0f), mDepth(0.0f), mRotation(0.0f),
        mOrigin{}, mScale(1.0f, 1.0f)
    {
    }
    
    AnimatedTexture(const DirectX::XMFLOAT2& origin, float rotation, float scale, float depth) noexcept :
        mPaused(false), mFrame(0), mFrameCount(0), mTextureWidth(0), mTextureHeight(0),
        mTimePerFrame(0.f), mTotalElapsed(0.f), mDepth(depth), mRotation(rotation), mOrigin(origin), mScale(scale, scale)
    {
    }
    ...
    void Load(ID3D11ShaderResourceView* texture, int frameCount, int framesPerSecond)
    {
        if (frameCount < 0 || framesPerSecond <=0)
            throw std::invalid_argument("AnimatedTexture expects a Texture2D");

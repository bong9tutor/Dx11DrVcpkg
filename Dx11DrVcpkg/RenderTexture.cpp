...
void RenderTexture::SetWindow(const RECT& outRect)
{
    // Determine the render target size in pixels
    const auto width  = size_t(std::max<LONG>(outRect.right - outRect.left, 1));
    const auto height = size_t(std::max<LONG>(outRect.bottom - outRect.top, 1));

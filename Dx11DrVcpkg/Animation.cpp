...
bool DX::AnimationSDKMESH::Bind(const DirectX::Model& model)
{
...
void DX::AnimationSDKMESH::Update(float delta)
{
    m_animTime += static_cast<double>(delta);
}

_Use_decl_annotations_
void DX::AnimationSDKMESH::Apply(const DirectX::Model& model, size_t nbones, DirectX::XMMATRIX* boneTransforms) const
{
    assert(m_animData && m_animSize > 0);
...
void AnimationCMO::Bind(const DirectX::Model& model)
{
    assert(!m_keys.empty());
...
_Use_decl_annotations_
void AnimationCMO::Apply(const DirectX::Model& model, size_t nbones, DirectX::XMMATRIX* boneTransforms) const
{
    assert(!m_keys.empty());
